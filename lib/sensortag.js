...
SensorTag.prototype.readTemperature = function(callback) {
  this.connection.addNotificationListener(0x25, function(err, attribute) {
    if (err) {
      callback(err, null);
    }

    // Calculations from http://processors.wiki.ti.com/index.php/SensorTag_User_Guide#IR_Temperature_Sensor
    var ambient = attribute.value.readUInt16LE(2) / 128.0;
    var Vobj2 = attribute.value.readInt16LE(0) * 0.00000015625;
...
SensorTag.prototype.readAccelerometer = function(callback) {
  this.connection.addNotificationListener(0x2D, function(err, attribute) {
    if (err) {
      callback(err, null);
    }

    // Calculations from http://processors.wiki.ti.com/index.php/SensorTag_User_Guide#Accelerometer_2
    var x = attribute.value.readInt8(0) / 64.0;
    var y = attribute.value.readInt8(1) / 64.0;
    var z = - attribute.value.readInt8(2) / 64.0;
...
SensorTag.prototype.readHumidity = function(callback) {
  this.connection.addNotificationListener(0x38, function(err, attribute) {
    if (err) {
      callback(err, null);
    }

    // From http://processors.wiki.ti.com/index.php/SensorTag_User_Guide#Humidity_Sensor_2
    var temp = -46.85 + 175.72/65536 * attribute.value.readInt16LE(0);
    var humidity = -6.0 + 125.0/65536 * (attribute.value.readUInt16LE(2) & ~0x0003)
...
SensorTag.prototype.readMagnetometer = function(callback) {
  this.connection.addNotificationListener(0x40, function(err, attribute) {
    if (err) {
      callback(err, null);
    }

    // From http://processors.wiki.ti.com/index.php/SensorTag_User_Guide#Magnetometer
    var x = - attribute.value.readInt16LE(0) / (65536/2000);
    var y = - attribute.value.readInt16LE(2) / (65536/2000);
    var z = attribute.value.readInt16LE(4) / (65536/2000);
...
SensorTag.prototype.calibrateBarometer = function() {
  ...
  var self = this;
  this.connection.readHandle(0x52, function(err, attribute) {
    // Calibration factors
    self.baromCalib[0] = attribute.value.readUInt16LE(0);
    self.baromCalib[1] = attribute.value.readUInt16LE(2);
    self.baromCalib[2] = attribute.value.readUInt16LE(4);
    self.baromCalib[3] = attribute.value.readUInt16LE(6);
    self.baromCalib[4] = attribute.value.readInt16LE(8);
    self.baromCalib[5] = attribute.value.readInt16LE(10);
    self.baromCalib[6] = attribute.value.readInt16LE(12);
    self.baromCalib[7] = attribute.value.readInt16LE(14);
  });
}
...
SensorTag.prototype.readBarometer = function(callback) {
  ...
  self.connection.addNotificationListener(0x4B, function(err, attribute) {
    if (err) {
      callback(err, null);
    }

    var rawTemp = attribute.value.readInt16LE(0);
    var rawPres = attribute.value.readUInt16LE(2);
...
SensorTag.prototype.calibrateGyroscope = function() {
  var self = this;
  this.connection.writeCommand(0x5B, new Buffer([7]));
  setTimeout(function() {
    self.connection.readHandle(0x57, function(err, attribute) {
      var rawY = attribute.value.readInt16LE(0);
      var rawX = attribute.value.readInt16LE(2);
      var rawZ = attribute.value.readInt16LE(4);
...
SensorTag.prototype.readGyroscope = function(x, y, z, callback) {
  var self = this;
  this.connection.addNotificationListener(0x57, function(err, attribute) {
    var rawY = attribute.value.readInt16LE(0);
    var rawX = attribute.value.readInt16LE(2);
    var rawZ = attribute.value.readInt16LE(4);
  ...
  if (x) enable |= 2;
  if (z) enable |= 4;
  this.connection.writeCommand(0x5B, new Buffer([enable])); // Configure gyroscope
  var buffer = new Buffer([1, 0]);
  this.connection.writeCommand(0x58, buffer); // Enable notification
}

SensorTag.prototype.checkPowerOnSelfTest = function(callback) {
  this.connection.readHandle(0x64, function(err, attribute) {
    var bitmask = attribute.value.readUInt16LE(0);
    callback(bitmask);
  });
}
