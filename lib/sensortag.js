...
SensorTag.prototype.calibrateBarometer = function() {
  ...
  this.connection.readHandle(0x52, function(err, buffer) {
    // Calibration factors
    self.baromCalib[0] = buffer.readUInt16LE(0);
    self.baromCalib[1] = buffer.readUInt16LE(2);
    self.baromCalib[2] = buffer.readUInt16LE(4);
    self.baromCalib[3] = buffer.readUInt16LE(6);
    self.baromCalib[4] = buffer.readInt16LE(8);
    self.baromCalib[5] = buffer.readInt16LE(10);
    self.baromCalib[6] = buffer.readInt16LE(12);
    self.baromCalib[7] = buffer.readInt16LE(14);
  });
}
...
SensorTag.prototype.calibrateGyroscope = function() {
  ...
  setTimeout(function() {
    self.connection.readHandle(0x57, function(err, buffer) {
      var rawY = buffer.readInt16LE(0);
      var rawX = buffer.readInt16LE(2);
      var rawZ = buffer.readInt16LE(4);
...
SensorTag.prototype.readGyroscope = function(x, y, z, callback) {
  var self = this;
  this.connection.addNotificationListener(0x57, function(err, buffer) {
    var rawY = buffer.readInt16LE(0);
    var rawX = buffer.readInt16LE(2);
    var rawZ = buffer.readInt16LE(4);
...
SensorTag.prototype.checkPowerOnSelfTest = function(callback) {
  this.connection.readHandle(0x64, function(err, buffer) {
    var bitmask = buffer.readUInt16LE(0);
    callback(bitmask);
  });
}
